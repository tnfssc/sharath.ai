...
// Every daisyUI theme enabled in styles.css, grouped by register.
const THEME_GROUPS: { label: string; themes: string[] }[] = [
  {
    label: 'Light',
    themes: ['light', 'cupcake', 'bumblebee', 'emerald', 'corporate', 'garden', 'pastel', 'fantasy', 'wireframe', 'cmyk', 'autumn', 'lemonade', 'winter', 'caramellatte', 'silk'],
  },
  {
    label: 'Dark',
    themes: ['dark', 'synthwave', 'retro', 'cyberpunk', 'valentine', 'halloween', 'forest', 'aqua', 'lofi', 'black', 'luxury', 'dracula', 'business', 'acid', 'night', 'coffee', 'dim', 'nord', 'sunset', 'abyss'],
  },
]
 ...
 */
function themeSwatches(theme: string): { bg: string; fg: string; primary: string } | null {
  if (typeof document === 'undefined') return null
  const el = document.createElement('div')
  el.setAttribute('data-theme', theme)
  el.style.cssText = 'position:absolute;left:-9999px;top:-9999px;width:1px;height:1px;visibility:hidden'
  document.body.appendChild(el)
  const cs = getComputedStyle(el)
  const sw = {
    bg: cs.getPropertyValue('--color-base-100').trim(),
    fg: cs.getPropertyValue('--color-base-content').trim(),
    primary: cs.getPropertyValue('--color-primary').trim(),
  }
  document.body.removeChild(el)
  // Check if the values resolved to real colors
  if (!sw.bg && !sw.primary) return null
  return sw
}

function Swatch({ theme }: { theme: string }) {
  const [colors, setColors] = useState<{ bg: string; fg: string; primary: string } | null>(null)
  ...
  if (!colors) {
    return <span className="inline-flex h-4 w-6 shrink-0 rounded-sm bg-base-300" />
  }
...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    setCurrent(document.documentElement.getAttribute('data-theme') ?? 'halloween')
  }, [])
  ...
  const isPrimary = (t: string) => PRIMARY_THEMES.includes(t)

  const moreThemes = useMemo(() => {
    const all = THEME_GROUPS.flatMap((g) => g.themes).filter((t) => !isPrimary(t))
    if (!query.trim()) return THEME_GROUPS.map((group) => ({
      ...group,
      themes: group.themes.filter((t) => !isPrimary(t)),
    })).filter((g) => g.themes.length > 0)
    const q = query.toLowerCase()
 return all.filter((t) => t.includes(q)).map((t) => ({ label: THEME_GROUPS.find((g) => g.themes.includes(t))!.label, themes: [t] })).reduce((acc: { label: string; themes: string[] }[], item) => {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        ...
        <ChevronDown className={`size-3 opacity-50 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-box border border-base-300 bg-base-100 shadow-xl">
          ...
          <div className="p-1">
            {PRIMARY_THEMES.map((theme) => (
              ...
              >
                <Swatch theme={theme} />
                <span className={`capitalize ${current === theme ? 'font-medium text-primary' : ''}`}>
          ...
          <div className="border-t border-base-300">
            ...
            >
              <span className={moreOpen ? 'text-primary' : ''}>More Themes</span>
              <ChevronDown className={`ml-auto size-3.5 opacity-50 transition-transform duration-200 ${moreOpen ? 'rotate-180' : ''}`} />
            </button>
          </div>
                  ...
                  {moreThemes.map((group) => (
                    <div key={group.label} className="mb-1 last:mb-0">
                      <div className="px-2 py-1 text-xs font-medium uppercase tracking-wider text-base-content/40">
                        {group.label}
                        <span className="ml-1.5 text-base-content/30">({group.themes.length})</span>
                      </div>
...
function SwatchLazy({ theme }: { theme: string }) {
  ...
  if (!colors) {
    return <span className="inline-block h-5 w-7 shrink-0 rounded-sm bg-base-300" />
  }
