...
type LoaderData = { post: BlogPost; html: string };

export const Route = createFileRoute("/blog/$slug")({
	head: ({ loaderData }: { loaderData?: LoaderData }) => ({
		// Loader throws notFound() when the post is missing, so loaderData is
		// always defined here — the guard only covers the preload window.
		meta: loaderData
			? [
					...seo({
						title: `${loaderData.post.title} — sharath.ai`,
						description: loaderData.post.description,
						image: loaderData.post.heroImage,
					}),
				]
